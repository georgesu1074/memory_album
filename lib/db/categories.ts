...
export async function updateCategorySummary(
  ...
  const { data: wedding } = await supabase
    .from('weddings')
    .select('groom_id, bride_id')
    .eq('id', weddingId)
    .single()
  
  // Get bride and groom names from detail records
  let groomName = 'Groom'
  let brideName = 'Bride'
  ...
  // Format couple names for display
  const coupleNames = `${groomName} and ${brideName}`
  ...
  if (memoriesError || !memories || memories.length === 0) {
    // No memories found, shouldn't happen but handle gracefully
    return
  }
